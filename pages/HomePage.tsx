...
const HomePage: React.FC<HomePageProps> = ({ issues, onSelectIssue, onReportIssue }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div>
            <div className="flex space-x-2 overflow-x-auto pt-2 pb-3 -mx-4 px-4">
              <button onClick={() => setActiveCategory('All')} className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap ${activeCategory === 'All' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200'}`}>
                All <span className="ml-1 bg-black/10 dark:bg-white/10 text-xs px-2 py-0.5 rounded-full">{getCategoryCount('All')}</span>
              </button>
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap ${activeCategory === cat ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200'}`}>
                  {cat} <span className="ml-1 bg-black/10 dark:bg-white/10 text-xs px-2 py-0.5 rounded-full">{getCategoryCount(cat)}</span>
                </button>
              ))}
            </div>
        </div>
      ...
      <div className="bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 p-3 rounded-lg flex items-center space-x-2 text-sm">
        ...
        <div className="space-y-4">
           {filteredIssues.length > 0 ? (
            ...
            <div className="text-center py-10">
              <p className="text-gray-600 dark:text-gray-400 font-semibold">No issues found</p>
