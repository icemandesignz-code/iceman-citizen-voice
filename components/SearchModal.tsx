        ...
        <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors border ${
                isActive
                    ...
                    : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-600'
            }`}
        >
            {label}
        </button>
...
const SearchModal: React.FC<SearchModalProps> = ({ onClose, issues, ministries, districts, onSelectIssue }) => {
    ...
    const renderResult = (item: any) => {
        switch (item.resultType) {
            ...
            case 'ministry':
                 ...
                 return (
                    <div key={`ministry-${ministry.id}`} className="p-3 my-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer transition-colors flex items-start space-x-3">
                        <MinistryIcon className="w-5 h-5 text-gray-500 dark:text-gray-400 mt-1 flex-shrink-0" />
                        <div>
                            <p className="font-semibold text-dark dark:text-white leading-tight">{ministry.name}</p>
                             <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{ministry.description}</p>
                        </div>
                    </div>
                 )
            case 'district':
                ...
                return (
                    <div key={`district-${district.id}`} className="p-3 my-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer transition-colors flex items-start space-x-3">
                        <DistrictsIcon className="w-5 h-5 text-gray-500 dark:text-gray-400 mt-1 flex-shrink-0" />
                        <div>
                            <p className="font-semibold text-dark dark:text-white leading-tight">{district.name}</p>
                             <p className="text-xs text-gray-500 dark:text-gray-400">{district.region}</p>
                        </div>
                    </div>
                )
            default:
    ...
    return (
        <div className="fixed inset-0 bg-white dark:bg-gray-800 z-50 flex flex-col animate-fade-in">
            <header className="p-4 flex-shrink-0 flex items-center space-x-2 border-b dark:border-gray-700">
                <div className="relative flex-grow">
                    ...
                    <input
                        ...
                        className="w-full bg-gray-100 dark:bg-gray-700 border-2 border-transparent rounded-full py-2 pl-12 pr-4 text-base focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary dark:text-white dark:placeholder-gray-400"
                        autoFocus
                    />
                </div>
                <button onClick={onClose} className="text-gray-600 dark:text-gray-300 hover:text-primary font-medium px-2">
                    Cancel
                </button>
            </header>
            ...
            <div className="p-4 flex-shrink-0 border-b dark:border-gray-700">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 overflow-x-auto pb-1">
                     ...
                     {showAdvancedFilterButton && <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} className={`p-2 rounded-full transition-colors ml-2 ${showAdvancedFilters ? 'bg-light text-primary' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600'}`}>
                        <FilterIcon className="w-5 h-5" />
                    </button>}
                </div>
                {showAdvancedFilters && showAdvancedFilterButton && (
                    <div className="mt-4 pt-4 border-t dark:border-gray-700 space-y-4 animate-fade-in-slow">
                        <div>
                            <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Status</h4>
                         ...
                         <div>
                            <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Category</h4>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div>
                             <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Date Range</h4>
                             <div className="flex items-center gap-2">
                                <input type="date" value={dateRange.from} onChange={e => setDateRange(p => ({...p, from: e.target.value}))} className="w-full px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm" placeholder="From" aria-label="From Date" />
                                <span className="text-gray-500 dark:text-gray-400">-</span>
                                <input type="date" value={dateRange.to} onChange={e => setDateRange(p => ({...p, to: e.target.value}))} className="w-full px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm" placeholder="To" aria-label="To Date" />
                             </div>
                        </div>
                    </div>
                )}
